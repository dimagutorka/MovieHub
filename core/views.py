...
def some_filters(request):
	num_comments = Movie.objects.annotate(num_comments=Count('comments')).order_by('-num_comments')
	avg_rating = Movie.objects.annotate(avg_rating=Avg('rates', default=1)).order_by('-avg_rating')
